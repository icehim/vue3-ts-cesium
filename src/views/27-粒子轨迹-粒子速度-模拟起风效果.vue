...
const particleSystem = () => {
  ...
  function applyGravity(particle: Particle, dt: number) {
    ...
    // 粒子正东方坐标
    const pos1 = Cartesian3.fromDegrees(longitude + 0.1, latitude + 0.1, height)
    let vectorSpeed = new Cartesian3(pos1.x - position.x, pos1.y - position.y, pos1.z - position.z)
    vectorSpeed = Cartesian3.normalize(vectorSpeed, vectorSpeed)
    ...
    // Cartesian3.multiplyByScalar(gravityVector, -9.8 * dt, gravityVector)
    particle.velocity = Cartesian3.add(particle.velocity, vectorSpeed, particle.velocity)
  }
