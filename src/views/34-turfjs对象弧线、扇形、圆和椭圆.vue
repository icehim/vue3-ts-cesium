...
import { featureCollection, circle, ellipse, lineArc, Feature, Geometry, sector } from '@turf/turf'
...
const createTurf = () => {
  // 圆 circle(中心点，半径，配置项)
  const circleFeature = circle([108.5, 34.5], 15, {
    steps: 100,
    units: 'kilometers',
    properties: { foo: 'bar' }
  })
  // 椭圆 ellipse(中心点，长轴长度，短轴长度，配置项)
  const ellipseFeature = ellipse([108.5, 35.5], 100, 50, { steps: 100, units: 'kilometers' })
  // 弧线
  const acrFeature: Feature<Geometry> = lineArc([108.5, 36], 5, 25, 180, {
    steps: 360,
    units: 'kilometers'
  })
  // 扇形
  const sectorFeatures = sector([108.5, 37], 5, 25, 90, {
    steps: 360,
    units: 'kilometers'
  })

  const collection = featureCollection([circleFeature, ellipseFeature, acrFeature, sectorFeatures])
