...
const load3DTileset = () => {
  const tileset = Cesium3DTileset.fromUrl('./data/3DTileset/tileset.json')
  tileset.then((tiles) => {
    // 原始点
    const boundingSphere = tiles.boundingSphere
    ...
    const offset = Cartesian3.fromRadians(longitude, latitude, height1)
    // 计算向量
    const translate = Cartesian3.subtract(offset, origin, new Cartesian3())
    // 矩阵偏移
    tiles.modelMatrix = Matrix4.fromTranslation(translate)
    // 开启深度测试
    viewerRef.value!.scene.globe.depthTestAgainstTerrain = true
    viewerRef.value?.scene.primitives.add(tiles)
    viewerRef.value?.zoomTo(tiles)
  })
}
