...
const groundPolylinePrimitive = () => {
  const position4Wgs84: Position[] = []
  // 申明一个XMLHttpRequest
  const request = new XMLHttpRequest()
  // 设置请求方法与路径
  request.open('get', '/data/hbLine.json')
  // 不发送数据到服务器
  request.send(null)
  // xhr对象获取到返回信息后执行
  request.onload = function () {
    // 解析获取到的数据
    const data: FeatureCollection<LineString> = JSON.parse(request.responseText)
    data.features.forEach((feature) => {
      feature.geometry.coordinates.forEach((coordinate) => {
        position4Wgs84.push(coordinate)
      })
    })
    const instance = new GeometryInstance({
      geometry: new GroundPolylineGeometry({
        positions: Cartesian3.fromDegreesArray(position4Wgs84.flat()),
        width: 4
      })
    })

    // 实现贴地线 使用groundPrimitives  会让没有高度的 Polyline geoJson数据 贴地
    viewerRef.value?.scene.groundPrimitives.add(
...
onMounted(async () => {
  ...
  hiddenCopyright()
  // setView故宫
  goSomewhere(118.142166, 41.2284, 100000.0)
  // 加载天地图文字标注图层
  loadMapFromTianditu(token, tdtUrl, subdomains)
  // 注册鼠标点击事件
  registerMouseEvent()
  // 贴底线 groundPolylinePrimitive
  groundPolylinePrimitive()
})
</script>
