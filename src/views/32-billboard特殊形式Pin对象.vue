...
const billboardPin = () => {
  ...
  const canvas = pinBuilder.fromColor(Color.ROYALBLUE, 48).toDataURL()
  ...
  // fromMakiIconId id在\node_modules\@cesium\engine\Source\Assets\Textures\maki 下
  const pinIconId = pinBuilder.fromMakiIconId('bus', Color.GREEN, 100) as Promise<HTMLCanvasElement>
  pinIconId.then((pinIconResult) => {
    viewerRef.value?.entities.add({
      name: 'pinIcon',
      position: Cartesian3.fromDegrees(117.391, 39.9163),
      billboard: {
        image: pinIconResult,
        verticalOrigin: VerticalOrigin.BOTTOM
      }
  ...
  // 文字
  const canvasText = pinBuilder.fromText('♥', Color.RED, 48).toDataURL()
  viewerRef.value?.entities.add({
    name: 'pinText',
    position: Cartesian3.fromDegrees(115.391, 39.9163),
    billboard: {
      image: canvasText,
      verticalOrigin: VerticalOrigin.BOTTOM
    }
  ...
  // url
  const pinUrl = pinBuilder.fromUrl(
    './src/assets/images/pin.svg',
    Color.HOTPINK,
    100
  ) as Promise<HTMLCanvasElement>
  pinUrl.then((pinIconResult) => {
    viewerRef.value?.entities.add({
      name: 'pinUrl',
      position: Cartesian3.fromDegrees(114.391, 39.9163),
      billboard: {
        image: pinIconResult,
        verticalOrigin: VerticalOrigin.BOTTOM
      }
