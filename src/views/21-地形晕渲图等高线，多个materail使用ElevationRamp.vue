...
const groundPolylinePrimitive = () => {
  const elevationCounterMaterial = new Material({
    fabric: {
      ...
      materials: {
        rampMaterial: {
          type: 'ElevationRamp',
          uniforms: {
            image: '/src/assets/images/gradientLine1.jpg',
            minimumHeight: 0,
            maximumHeight: 1600
          }
        },
      ...
      components: {
        diffuse: 'counterMaterial.alpha==0.0 ? rampMaterial.diffuse : counterMaterial.diffuse',
        alpha: 'max(counterMaterial.alpha,rampMaterial.alpha)'
      }
    },
    translucent: false
  })
  viewerRef.value!.scene.globe.material = elevationCounterMaterial
}
onMounted(async () => {
  // 天地图token
