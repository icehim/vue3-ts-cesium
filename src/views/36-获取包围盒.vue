...
const createTurf = () => {
  ...
  const bbox1 = bbox(polygon1)

  const bboxPolygon1 = bboxPolygon(bbox1)
  console.log('bboxPolygon', bboxPolygon1)

  const collection = featureCollection([polygon1, centroid1, bboxPolygon1])

  const envelope1 = envelope(collection)

  const dataSource = GeoJsonDataSource.load(envelope1, {
    stroke: Color.BLUE,
    fill: Color.RED,
    strokeWidth: 3,
    markerSymbol: '?'
  ...
  const dataSource1 = GeoJsonDataSource.load(collection, {
    stroke: Color.BLUE,
    fill: Color.RED,
    strokeWidth: 3,
    markerSymbol: '?'
  })
  dataSource1.then((data) => {
    viewerRef.value?.dataSources.add(data)
  })
}
